...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            undo = input("Do you want to undo your move")
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            toCheck = [(0,0),(0,2),(-2,0),(-2,2)]
                            for row,col in toCheck:
                                if not self.__GetCell(StartRow+row,StartColumn+col).CheckSymbolAllowed(self.__GetCell(StartRow+row,StartColumn+col).GetSymbol()):
                                    break
                            else:
    ...
    def savePuzzle(self):
        ...
        for i in self.__AllowedPatterns:
            data += "\n"
            data += i.getPatternSymbol()
            data += ","
            data += i.GetPatternSequence()
        ...
        for cell in self.__Grid:
            ...
            if symbol != "-":
                data += cell.GetSymbol()
            notAllowed = cell.getNotAllowedSymbols()
           
            data +=","
            data += ",".join(notAllowed)
            data += "\n"
...
class PreviousMove(Cell):
    def __init__(self,location, prevSymbol,prevSymbolsNotAllowed):
        super().__init__()
        self.location = location
        for i in prevSymbolsNotAllowed:
            self.AddToNotAllowedSymbols(i)
        self.ChangeSymbolInCell(prevSymbol)
