...
class CinemaOutlet(Outlet):
  def __init__(self, XCoord, YCoord, MaxCapacityBase):
    super().__init__(XCoord, YCoord, MaxCapacityBase)
  
  def CalculateDailyProfitLoss(self, AvgCostPerMeal, AvgPricePerMeal):
    return (AvgPricePerMeal*2-AvgCostPerMeal) * self._VisitsToday - self._DailyCosts
  
  def GetDetails(self):
    parent = super().GetDetails()
    parent += "\t Cinema Outlet"
    return parent
...
class Company:
  ...
  def OpenOutlet(self, X, Y):
    ...
    if random.random() <= 0.1:
      NewOutlet = CinemaOutlet(X,Y,self._Capacity)
    else:
...
class Simulation:
  def __init__(self):
    ...
    Choice = input("Enter L for a large settlement, anything else for a normal size settlement: ").upper()
    if Choice == "L":
    ...
    Choice = input("Enter D for default companies, anything else to add your own start companies: ").upper()
    if Choice == "D":
  ...
  def ProcessDayEnd(self,multiple):
    TotalReputation = 0.0
    Reputations = []
    for C in self._Companies:
      C.NewDay()
  ...
  def Run(self):
    ...
    while Choice.upper() != "Q":
      ...
      if Choice == "1":
        ...
        for i in range(days):
          self.ProcessDayEnd(multiple=1)
        
      elif Choice == "6":
        self.ProcessDayEnd(multiple=0)
      elif Choice.upper() == "Q":
